...
key <- "AIzaSyBDzZCc8sw2IC36F4trV4a6Z3_w_DiAmRU"
videoid <- "tAE83zZEcOY" #神級陪玩「胖貓」遭詐乾227萬亡 女友道歉｜20240506 ET午間新聞
...
parse_comment_snippet <- function(snippet, parentId = NA_character_) {
  tibble(
    ...
    commentId    = snippet$commentId %||% NA_character_,
    parentId     = parentId,
    author       = snippet$authorDisplayName %||% NA_character_,
    text         = enc2utf8(snippet$textOriginal %||% snippet$textDisplay %||% NA_character_),
...
# Save the cleaned comments to a CSV file in the "Final_project_data" folder
write.csv(
  comments,
  file = "Final_project_data/CN_SIMP005_comments.csv",
  row.names = FALSE,
  fileEncoding = "UTF-8"
)
...
comments <- comments |>
  mutate(text = str_squish(text)) |>
  mutate(text = enc2utf8(text))  # ensure Chinese stays readable
